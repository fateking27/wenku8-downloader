...
const dir = `${process.cwd()}/@lesjoursfr/html-to-epub`
export class EPub {
    uuid;
    title;
    description;
    ...
    constructor(options, output) {
        ...
        for (const collection of this.collections) {
            if (collection.type && !["series", "set"].includes(collection.type)) {
                throw new Error(`Invalid collections: ${this.collections.map((c) => `${c.name}: ${c.type}`).join(", ")}. Allowed types are "series" and "set".`);
            }
        }
        ...
        this.tempDir = options.tempDir ?? join(dir, "/tempDir/");
        ...
        if (this.cover) {
            const templatePath = this.customHtmlCoverTemplatePath || join(dir, `/templates/epub${this.version}/cover.xhtml.ejs`);
        ...
        const contentTemplatePath = join(dir, "/templates/content.xhtml.ejs");
    ...
    validateElement(contentIndex, node) {
        ...
        if (this.version === 2) {
            if (!this.allowedXhtml11Tags.includes(node.tagName)) {
                if (this.verbose) {
                    console.log(`[Warning] (content[${contentIndex}]) ${node.tagName} tag isn't allowed on EPUB 2/XHTML 1.1 DTD.`);
                }
                node.tagName = "div";
            }
        }
    }
    processMediaTag(contentIndex, dir, node, mediaArray, subfolder) {
        ...
        if (media) {
            ...
            if (mediaType === null) {
                if (this.verbose) {
                    console.error(`[Media Error] (content[${contentIndex}]) (subfolder=${subfolder}) The media can't be processed : ${url}`);
                }
                return;
            }
...
`;
        // Copy the CSS style
        if (!this.css) {
            this.css = readFileSync(join(dir, "/templates/template.css"), { encoding: "utf8" });
        }
...
`);
        ...
        const opfPath = this.customOpfTemplatePath || join(dir, `/templates/epub${this.version}/content.opf.ejs`);
        ...
        const ncxTocPath = this.customNcxTocTemplatePath || join(dir, "/templates/toc.ncx.ejs");
        ...
        const htmlTocPath = this.customHtmlTocTemplatePath || join(dir, `/templates/epub${this.version}/toc.xhtml.ejs`);
